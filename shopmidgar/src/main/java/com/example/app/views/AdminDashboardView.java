...
public class AdminDashboardView extends BorderPane {
    ...
    private Label lblUniversTotals;
    private Label lblUniversMed;
    ...
    private void refreshStats() {
        try {
            ...
            List<com.example.app.entities.Universe> all = us.select();
            lblUniversTotals.setText(String.valueOf(all.size()));
            
            // Just displaying something dynamic for the second stat
            long fantasyCount = all.stream().filter(u -> "Fantasy".equalsIgnoreCase(u.getGenre())).count();
            lblUniversMed.setText(String.valueOf(fantasyCount));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private void setupMainContent() {
        ...
        lblUniversTotals = new Label("0");
        lblUniversTotals.setStyle("-fx-text-fill: #18E3A4; -fx-font-size: 32px; -fx-font-weight: bold;");
        
        lblUniversMed = new Label("0");
        lblUniversMed.setStyle("-fx-text-fill: #F39C12; -fx-font-size: 32px; -fx-font-weight: bold;");

        // Stat Cards HBox
        HBox statsBox = new HBox(20);
        statsBox.getChildren().addAll(
            createStatCard("Univers Totals", lblUniversTotals, "#18E3A4"),
            createStatCard("Univers Fantasy", lblUniversMed, "#F39C12")
        );
        ...
        Button mainBtn = new Button("+ Ajouter Univers");
        ...
        mainBtn.setOnAction(e -> getScene().setRoot(new UniverseCreateView(null, true)));
