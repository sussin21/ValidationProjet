...
public class PersonnageTableView extends TableView<Personnage> {
    ...
    private void setupTable() {
        ...
        TableColumn<Personnage, String> univCol = new TableColumn<>("Univers");
        univCol.setCellValueFactory(new PropertyValueFactory<>("univers"));
        univCol.setPrefWidth(150);
        ...
        this.getColumns().addAll(idCol, nameCol, classCol, univCol, actionsCol);
    }

    private void setupActionsColumn(TableColumn<Personnage, Void> actionsCol) {
        actionsCol.setCellFactory(param -> new TableCell<Personnage, Void>() {
            ...
            
            {
                ...
                editBtn.setOnAction(e -> {
                    Personnage data = getTableView().getItems().get(getIndex());
                    System.out.println("Éditer Personnage: " + data.getName());
                    getScene().setRoot(new PersonnageCreateView(data, true));
                });
                
                deleteBtn.setOnAction(e -> {
                    Personnage data = getTableView().getItems().get(getIndex());
                    System.out.println("Supprimer Personnage: " + data.getName());
                    try {
                        personnageService.delete(data.getId());
                        loadData(); 
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
    ...
    public void loadData() {
        try {
            List<Personnage> data = personnageService.select();
    ...
    public void search(String query) {
        try {
            List<Personnage> data = personnageService.searchPersonnages(query, null, null, "Récents");
            this.getItems().setAll(data);
        } catch (Exception e) {
            e.printStackTrace();
        }
